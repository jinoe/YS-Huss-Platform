...
- `src/style.css` — 전역 스타일, 색상 변수. **팔레트는 절제된 톤 하나**: `--color-primary`/`--color-primary-dark`(네이비, 큰 그라데이션 배너 전용) + `--color-accent`(블루, 링크/active 상태 전용) + `--color-text`/`--color-muted`/`--color-border` + `--bg-soft`(연한 회색, 카드/섹션 배경). 초록/올리브/노랑 등 여러 색을 섞어 쓰지 않음 — 배경은 흰색/연회색 위주로, 포인트 컬러는 최소한으로만
